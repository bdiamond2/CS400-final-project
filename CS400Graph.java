...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    public Edge getEdge(T source, T target) {
        if (source == null || target == null)
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            this(copyPath.start);
            Vertex prevVertex = this.start;

            // add all of copyPath's remaining vertices
            // start at 1 because we already added index 0
            for (int i = 1; i < copyPath.dataSequence.size(); i++) {
                T nextData = copyPath.dataSequence.get(i);
                this.dataSequence.add(nextData);
                boolean foundNextEdge = false;
                // find the edge that's going to this next vertex
                for (Edge e : prevVertex.edgesLeaving) {
                    if (e.target.data.equals(nextData)) {
                        this.distance += e.weight;
                        prevVertex = e.target;
                        foundNextEdge = true;
                        break;
                    }
                ...
                if (!foundNextEdge) {
                    throw new IllegalStateException("Invalid data sequence");
                }
            }
            // add the extension vertex
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        if (startVertex == null) {
            throw new NoSuchElementException("Invalid start vertex");
        }
        if (this.vertices.get(end) == null) {
            throw new NoSuchElementException("Invalid end vertex");
        }

        Path p = new Path(startVertex);
        // if it's a path to itself then it's a trivial zero-length path
        if (start.equals(end)) {
            return p;
        }
        ...
        // Put initial path(s) in the queue before starting the loop
        // This is necessary so you can enqueue a path's next iteration(s) before
        // dequeueing it.
        // A do-while loop will NOT work, you have to enqueue twice before dequeueing
        ...
        while (q.size() > 0) {
            // 1. Gather the paths leading away from start
            for (Edge e : p.end.edgesLeaving) {
                // 2. Put them in a priority queue
                if (!visited.contains(e.target)) {
                    q.add(new Path(p, e));
                }
            }
            // 3. Pop off the shortest path
            p = q.poll();
            // 4. Stop when the end of a chosen path is the end node
